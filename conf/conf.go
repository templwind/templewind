...
func LoadFromJsonBytes(content []byte, v any) error {
	info, err := buildFieldsInfo(reflect.TypeOf(v), "", make(map[reflect.Type]bool))
	if err != nil {
		return err
	}
...
func buildAnonymousFieldInfo(info *fieldInfo, lowerCaseName string, ft reflect.Type, fullName string, visited map[reflect.Type]bool) error {
	switch ft.Kind() {
	case reflect.Struct:
...
func buildFieldsInfo(tp reflect.Type, fullName string, visited map[reflect.Type]bool) (*fieldInfo, error) {
	tp = mapping.Deref(tp)

	// Only track struct types in visited
	if tp.Kind() == reflect.Struct {
		...
		visited[tp] = true
		defer func() { delete(visited, tp) }()
	}

	switch tp.Kind() {
	case reflect.Struct:
		return buildStructFieldsInfo(tp, fullName, visited)
...
func buildNamedFieldInfo(info *fieldInfo, lowerCaseName string, ft reflect.Type, fullName string, visited map[reflect.Type]bool) error {
	var finfo *fieldInfo
	var err error
...
func buildStructFieldsInfo(tp reflect.Type, fullName string, visited map[reflect.Type]bool) (*fieldInfo, error) {
	info := &fieldInfo{
		children: make(map[string]*fieldInfo),
	}
